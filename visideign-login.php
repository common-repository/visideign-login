<?php
/*
Plugin Name: visideign Login 
Plugin URI: http://www.visideign.com/plugins/visideign-wplogin
Description: visdideign login plugin provide support to you to facilite your visiter to register with you and securley perform activities on your wordpress Blog, Website or portal.
Version: 1.0
Author: visideign
Author URI: http://www.visideign.com
*/

/*
Copyright (C) 2014  visideign
...
$nd_login_vars['plugin_path'] = WP_PLUGIN_DIR.'/visideign-login';
$nd_login_vars['plugin_url'] = WP_PLUGIN_URL.'/visideign-login';

load_plugin_textdomain('ninety', $nd_login_vars['plugin_url'] . '/langs/', 'visideign-login/langs/');
...
function nd_login_init_style() {
	...
	if (file_exists(TEMPLATEPATH . '/visideign-login/login.css')) $logincss = get_bloginfo('template_url') . '/visideign-login/login.css';
...
function nd_login_init() {
	class NinetyAjaxLogin extends WP_Widget {
	    function NinetyAjaxLogin() {  
			...
			$this->WP_Widget('nd_ajax_login', __('visideign Login','ninety'), $widget_ops);  
	    }
...
function nd_login_load_template( $name ) {
	global $nd_login_vars;
	if (file_exists(TEMPLATEPATH . '/visideign-login/' . $name)) include( TEMPLATEPATH . '/visideign-login/' . $name ); else include( $nd_login_vars['plugin_path'] . '/template/' . $name );
}

###[ Shortcode ]###############################################WebCareZone.com####

add_shortcode( 'visideignlogin', 'nd_login_widget' );
